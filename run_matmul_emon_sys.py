...
def run_matmul(src_row, src_col, weight_row, weight_col, batch_size, threads, layer, total_mem_mb, cacheinput, args ):
    ...
    return float(time_str), float(socket1)
                ...
                for threads in ompthreads_values:
                    for skew in skew_values:
                        ...
                        if status == 0:
                            ...
                            print('An error occurred')
                        execution_time, S1 = run_matmul(src_row, src_col, weight_row, weight_col, batch_size, threads, layer, total_mem_mb, cacheinput, args)
                        time.sleep(2)
                        ...
                        file.write(f"{threads},{skew},{src_mem_kb},{total_mem_mb},{execution_time},{S1},{cacheinput},{str(args.noamx).lower()}\n")
